...
export interface EventLogParser {
  parseEventLog: (_: RawEventLog) => EventLogEntry
}

export function buildEventLogParser(game: GameState): EventLogParser {
  const actionCardsByName = uniqueGroupBy(game.actionCards, (c) => c.name)
  const shipsByName = uniqueGroupBy(game.shipCards, (c) => c.name)
  const commandShipsByName = uniqueGroupBy(game.commandShipCards, (c) => c.name)

  return {
    parseEventLog(r: RawEventLog): EventLogEntry {

...
import { ChooseShipAction, ActionError, PlayerId } from '../shared-types';
import { stringList, warn } from '../utils';
import {
  canRespondToBlast, resolveBlastAttack,
  shipCanFire,
  ...
  playersThatCanRespondToActions, squadronDamage,
  minesweeperTargets,
...
export function applyChooseShipAction(
  state: GameState,
  playerId: PlayerId,
  action: ChooseShipAction): ActionError | undefined {
  ...
  switch (state.turnState.type) {
    case 'ManeuverTurnState': {
      ...
      if (!moveableShips(playerId, activePlayerState).some((shipIndex) => _.isEqual(shipIndex, action.choice)
      )) {
    ...
    case 'AttackChoosePlayerToMinesweepState':
      {
        ...
        const resolveActivation = () => {
          state.pushEventLog(
            event`${state.activePlayer} activates their Minesweeper ${minesweeper.shipType.name} on ${targetPlayer === state.activePlayer
                ? 'themselves'
                : p(targetPlayer)}!`
          );
        ...
        if (targetPlayerState.asteroidsUntilBeginningOfPlayerTurn !== undefined &&
          targetPlayerState.minefieldUntilBeginningOfPlayerTurn !== undefined) {
          state.turnState = {
        ...
        } else if (targetPlayerState.asteroidsUntilBeginningOfPlayerTurn !== undefined) {
          resolveAsteroids();
        } else if (targetPlayerState.minefieldUntilBeginningOfPlayerTurn !== undefined) {
          resolveMinefield();
        } else {
    ...
    case 'AttackChooseAsteroidsPlayerTurnState':
      {
        ...
        state.pushEventLog(
          event`${p(state.activePlayer)} plays ${'Asteroids'} on ${targetPlayer === state.activePlayer ? 'themselves' : p(targetPlayer)}.`
        );
    ...
    case 'AttackChooseMinefieldPlayerTurnState':
      {
        ...
        state.pushEventLog(
          event`${p(state.activePlayer)} plays a ${'Minefield'} on ${targetPlayer === state.activePlayer ? 'themselves' : p(targetPlayer)}.`
        );
    ...
    case 'AttackChooseSpacedockShipState':
      {
        ...
        const targetShip = state.getPlayerState(targetPlayer).ships[action.choice[1]];
        ...
        state.pushEventLog(
          event`${p(state.activePlayer)} plays ${'Spacedock'} on ${targetShip.shipType.name}.`
        );
    ...
    case 'PlayBlastChooseTargetShipState':
      {
        ...
        if (Array.isArray(action.choice)) {
          if (!blastableShipIndices(state, state.turnState.firingShip).some(
          ...
          if (!blastableCommandShipPlayers(
            state,
        ...
        if (state.turnState.blast.cardType === 'RammingSpeedCard') {
          state.pushEventLog(
            event`${p(state.activePlayer)}'s ${state.turnState.firingShip.shipType.name} rams into ${p(targetPlayerId)}'s ${designatedShip.shipType.name}, dealing ${state.turnState.firingShip.shipType.movement} damage.`
          );
        } else {
          state.pushEventLog(
            event`${p(state.activePlayer)}'s ${state.turnState.firingShip.shipType.name} fires a ${state.turnState.blast.name} at ${p(targetPlayerId)}'s ${designatedShip.shipType.name}, dealing ${state.turnState.blast.damage} damage.`
          );
        }
        ...
        if (targetPlayerState.hand.some(canRespondToBlast)) {
          state.turnState = {
            ...
            resolveBlast: () => resolveBlastAttack(
              state,
    ...
    case 'PlaySquadronChooseTargetShipState':
      {
        ...
        if (Array.isArray(action.choice)) {
          ...
          if (!squadronableCommandShipPlayers(
            state,
            playerId,
            state.turnState.squadron
          ).includes(action.choice)) {
        ...
        state.pushEventLog(
          event`${p(state.activePlayer)} deploys a ${squadron.name} targeting ${p(targetPlayerId)}'s ${designatedShip.shipType.name}, dealing ${squadronDamage(state, designatedShip, squadron)} damage.`
        );

        if (targetPlayerState.hand.some((c) => canRespondToSquadron(targetPlayerState, c)
        )) {
          state.turnState = {
            type: 'PlaySquadronRespondState',
            squadron,
            targetShip: designatedShip,
            resolveSquadron: () => resolveSquadronAttack(state, designatedShip, squadron),
          };
        } else {
